use darklua_core::rules::{ConvertLuaxToLua, Rule};

test_rule!(
    convert_luax_to_lua,
    ConvertLuaxToLua::default(),
    simple_test("<Comp><Comp/>") => "React.createElement(Comp, nil, nil)",
...
fn deserialize_from_string() {
    json5::from_str::<Box<dyn Rule>>("'convert_luax_to_lua'").unwrap();
}
